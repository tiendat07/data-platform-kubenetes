...
def load_config(spark_context: SparkContext):
    ...
    spark_context._jsc.hadoopConfiguration().set("fs.s3a.endpoint", S3_HTTP_ENDPOINT)
    spark_context._jsc.hadoopConfiguration().set(
...
def get_spark(
    ...
    conf = (
        ...
        .set("spark.eventLog.dir", SPARK_EVENT_LOG_DIR)
        .set("spark.hadoop.fs.s3a.endpoint", S3_HTTP_ENDPOINT)
        .set("spark.hadoop.fs.s3a.access.key", AWS_ACCESS_KEY_ID)
        .set("spark.hadoop.fs.s3a.secret.key", AWS_SECRET_ACCESS_KEY)
        .set("spark.hadoop.fs.s3a.path.style.access", S3_PATH_STYLE_ACCESS)
        .set("spark.hadoop.fs.s3a.connection.ssl.enabled", S3_SSL_ENABLE)
        .set(
        ...
        .set("spark.sql.catalog.lakehouse.s3.endpoint", S3_HTTP_ENDPOINT)
        .set("spark.sql.defaultCatalog", "lakehouse")  # Name of the Iceberg catalog
        .set("spark.sql.catalogImplementation", "in-memory")
        .set("spark.sql.catalog.lakehouse.type", "rest")  # Iceberg catalog type
        .set("spark.sql.catalog.lakehouse.uri", ICEBERG_REST_CATALOG_URI)
